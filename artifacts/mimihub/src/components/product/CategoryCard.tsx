...
export function CategoryCard({ category, className = '' }: CategoryCardProps) {
  ...
  return (
    <Link href={`/category/${category.slug}`} className={`block ${className}`}>
      <article className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-secondary shadow-sm group">
        <img
          ...
          {subcategories.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-1.5">
              ...
              {subcategories.length > 4 && (
                <span className="rounded-full bg-white/20 px-2.5 py-1 text-[10px] backdrop-blur-sm">
                  +{subcategories.length - 4} more
                </span>
              )}
            </div>
          )}
          <span className="mt-4 inline-flex items-center text-xs font-semibold uppercase tracking-wider transition-colors group-hover:text-primary">
